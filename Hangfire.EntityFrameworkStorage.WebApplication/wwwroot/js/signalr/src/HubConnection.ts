    ...
    stream<T = any>(methodName: string, ...args: any[]): IStreamResult<T> {
        ...
        subject.cancelCallback = () => {
            const cancelInvocation: CancelInvocationMessage =
                this._createCancelInvocation(invocationDescriptor.invocationId);
...
_connectionClosed(error ? : Error);
{
    ...
    this._cancelCallbacksWithError(error ||
...
_reconnect(error ? : Error);
{
    ...
    while (nextRetryDelay !== null) {
        ...
        try {
            ...
            if (this._connectionState !== HubConnectionState.Reconnecting) {
                this._logger.log(LogLevel.Debug,
                    `Connection moved to the '${this._connectionState
            ...
            nextRetryDelay =
                this._getNextRetryDelay(previousReconnectAttempts++, Date.now() - reconnectStartTime, retryError);
        }
    }
...
private;
_getNextRetryDelay(previousRetryCount:;
...
private;
_launchStreams(streams:;
IStreamResult < any > [], promiseQueue:;
Promise < void > );:
void {
    if (streams.length === 0);
{
...
for (const streamId in streams) {
    streams[streamId].subscribe({
        ...
        error: (err) => {
            ...
            promiseQueue =
                promiseQueue.then(() => this._sendWithProtocol(this._createCompletionMessage(streamId, message)));
        },
        next: (item) => {
            promiseQueue =
                promiseQueue.then(() => this._sendWithProtocol(this._createStreamItemMessage(streamId, item)));
        },
    });
}
}
...
private;
_createStreamInvocation(methodName:;
string, args:;
any[], streamIds:;
string[]);:
