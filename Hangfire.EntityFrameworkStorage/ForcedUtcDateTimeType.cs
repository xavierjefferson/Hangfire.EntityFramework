...
namespace Hangfire.EntityFrameworkStorage
{
    ...
    public class ForcedUtcDateTimeType : DateTimeType
    {
        ...
        /// </remarks>
        public override void Set(DbCommand st, object value, int index, ISessionImplementor dbContext)
        {
            if (value == null)
            {
                base.Set(st, null, index, dbContext);
                return;
            }
        ...
        public override object Get(DbDataReader rs, int index, ISessionImplementor dbContext)
        {
            return ChangeDateTimeKindToUtc(base.Get(rs, index, dbContext));
        }

        public override object Get(DbDataReader rs, string name, ISessionImplementor dbContext)
        {
            return ChangeDateTimeKindToUtc(base.Get(rs, name, dbContext));
        }
