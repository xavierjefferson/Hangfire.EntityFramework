            ...
            WithLock(deletionArgs, nameof(_DistributedLock), () =>
            {
                return DeleteEntities<_DistributedLock>((dbContext, cutoff) =>
                {
                    ...
                    var idList = dbContext.DistributedLocks
                        ...
                        .Select(i => i.Id)
                        .ToList();
                    return dbContext.DeleteByInt32Id<_DistributedLock>(idList);
                });
            });
        }
...
#pragma warning restore 618
...

        {
            ...
            {
                var idList = dbContext.Query<T>().Where(i => i.Job.ExpireAt < cutoff)
                    .Take(info.NumberOfRecordsInSinglePass)
                    .Select(i => i.Id)
                    .ToList();
                return dbContext.DeleteByInt32Id<T>(idList);
            });
        }
        ...
        private long DeleteExpirableEntity<T>(DeletionArgs info) where T : class, IExpirableWithId
        {
            ...
            {
                var ids = dbContext.Query<T>()
                    .Where(i => i.ExpireAt < cutoff).Take(info.NumberOfRecordsInSinglePass)
                    .Select(i => i.Id)
                    .ToList();
                return dbContext.DeleteByInt32Id<T>(ids);
            });
        }
        ...
        public long DeleteEntities<T>(Func<HangfireContext, DateTime, long> deleteFunc) where T : class
